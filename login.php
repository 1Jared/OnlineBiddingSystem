...
error_reporting(E_ALL);
	...
	if (isset($_POST['next2'])){
				...
				mysqli_query($con,"INSERT INTO member(lastname,firstname,gender,userid,password,email,contactno,birthdate,address,verification,memberimg) VALUES ('$lname','$fname','$gender','$userid','$password','$email','$contactno','$birthdate','$cusaddress','no','default.jpg')");
				$query = mysqli_query($con,"SELECT * FROM member WHERE userid = '$userid'") or die (mysqli_error());
					$row = mysqli_fetch_array($query);
					$id= $row['memberid'];
					mysqli_query($con,"INSERT into secretquestions(memberid,question,answer) VALUES ('$id','$secques','$secanswer')");
				$_SESSION['ID']= $id;
	...
	if(isset($_POST['login'])){
				if(isset($_POST['user'])) {
					if(isset($_POST['pass'])){			
						$username = $_POST['user'];	
						$pass = $_POST['pass'];
						$query = mysqli_query($con,"SELECT * FROM member WHERE userid = '$username' AND  password = '$pass'") or die (mysqli_error());
