...
<body>
	...
	<?php
		...
		$id = $_GET['id'];
		$pname = mysqli_query($con,"SELECT * FROM products WHERE productid='$id'")or die(mysqli_error());
	...
	?>
        ...
        <tbody>
        	<?php 
				...
				$con=mysqli_connect('localhost', 'root', '', 'biddingsystemdb');
				$prodid = $_GET['id'];
				$query = mysqli_query($con,"SELECT * FROM bidreport LEFT JOIN member ON member.memberid = bidreport.bidder LEFT JOIN products ON products.productid = bidreport.productid WHERE products.productid = '$prodid'") or die(mysqli_error());
