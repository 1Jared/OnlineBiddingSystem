...
error_reporting(E_ALL);
  ...
  $con=mysqli_connect('localhost', 'root', '', 'biddingsystemdb');
	$query = mysqli_query($con,"SELECT * FROM products WHERE status = 0") or die (mysqli_error());
	while($row = mysqli_fetch_array($query))
	{
		...
		if($datenow >= $duedate){
			mysqli_query($con,"UPDATE products SET status = 1 WHERE productid = '$prodid'") or die (mysqli_error());
		}
	}
	headhtml();
?>
