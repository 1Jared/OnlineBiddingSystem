	...
	<div id="container">
		<div id="bgwrap">
			...
			<center><h1><br />Welcome Administrator</h1></center>
				...
				$endedsum = mysqli_query($con,"SELECT * FROM products WHERE status = 1") or die(mysqli_error());
						$counters = 0;
				...
				<!--end query-->
					...
					error_reporting(E_ALL);
					$con=mysqli_connect('localhost', 'root', '', 'biddingsystemdb');
					$bids_stat = mysqli_query($con,"SELECT * FROM bidreport LEFT JOIN member ON member.memberid = bidreport.bidder LEFT JOIN products ON products.productid = bidreport.productid WHERE bidreport.status = 0") or die(mysql_error());
					...
					error_reporting(E_ALL);
					$con=mysqli_connect('localhost', 'root', '', 'biddingsystemdb');
					$endcheck = mysqli_query("SELECT * FROM products WHERE status = 1") or die(mysqli_error());
						...
						if ($numberOfRows == 0)
							...
							while($row2 = mysqli_fetch_array($query2)){
								...
								$query3 = mysqli_query($con,"SELECT * FROM bidreport WHERE bidamount = '$higestbid' AND productid = '$prodid' ") or die (mysqli_error());
								$winner=mysqli_fetch_array($query3);
								$hewins=$winner['bidder'];
								mysqli_query($con,"UPDATE products SET status = 0 WHERE productid = '$prodid' ") or die (mysqli_error());
								$rows = mysqli_query($con,"SELECT * needtopay WHERE memberid = '$hewins' AND productid = '$prodid'");
								$numrows1 = mysqli_num_rows($rows);
								if ($numrows1 == 0){
								mysqli_query($con,"INSERT INTO needtopay(memberid,productid,status,payment,declined,dateadded) VALUES('$hewins','$prodid','0','$higestbid','0',NOW()) ") or die (mysqli_error());
								}
							}
					?> 
