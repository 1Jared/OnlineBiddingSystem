...
//shows the add to watchlist option--------
	...
	function watsnew(){
		$con=mysqli_connect('localhost', 'root', '', 'biddingsystemdb');
		$query = mysqli_query($con,"SELECT * FROM products WHERE status = 0 ORDER BY productid DESC LIMIT 0,1") or die (mysqli_error());
		while($row = mysqli_fetch_array($query)){
		echo 
...
//shows latest products-----
	...
	function latest(){
		$con=mysqli_connect('localhost', 'root', '', 'biddingsystemdb');
		$query = mysqli_query($con,"SELECT * FROM products WHERE status = 0 ORDER BY productid DESC LIMIT 0,6") or die (mysqli_error());
...
//shows products on a category-----
	...
	function showprod(){
		$con=mysqli_connect('localhost', 'root', '', 'biddingsystemdb');
		$id = $_GET['id'];
		$query = mysqli_query($con,"SELECT * FROM products WHERE categoryid = '$id' AND status = 0") or die (mysqli_error());
...
//shows the products on watch--------
 function onwatch(){
	$con=mysqli_connect('localhost', 'root', '', 'biddingsystemdb');
		...
		$query1 = mysqli_query($con,"SELECT * FROM watchlist WHERE memberid = '$who_u'")or die(mysqli_error());
		while($row1 = mysqli_fetch_array($query1)){
			$prod = $row1['productid'];
			$query = mysqli_query($con,"SELECT * FROM products WHERE productid = '$prod'")or die(mysqli_error());
			while($row = mysqli_fetch_array($query)){
				echo "<div class='prod_box'>";
					echo "<div class='top_prod_box'></div>";
					echo "<div class='center_prod_box'>";
